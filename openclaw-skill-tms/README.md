...
This **OpenClaw skill** talks to the same TMS backend over its REST API. Instead of using the Telegram bot, you can ask your OpenClaw agent to add downloads, list status, remove a download, or search torrents — all via natural language in chat. The default API URL is `http://127.0.0.1:8080` for same-host installs. If `TMS_API_KEY` is configured, the agent must use it on every request; the key should live in OpenClaw skill env or `apiKey`, not in chat. The skill does not install any code or run binaries.
...
- A running **Telegram Media Server** with the API enabled (`TMS_API_ENABLED=true`) and network access from the machine where OpenClaw runs. If TMS has `TMS_API_KEY` set, OpenClaw must send it on every request, even for same-host installs. Ansible-generated TMS installs set this key by default.
- **Environment variables** for the agent: `TMS_API_URL` is optional for same-host installs and defaults to `http://127.0.0.1:8080`. `TMS_API_KEY` is required whenever TMS has API auth enabled or generated by Ansible; store it in skill env or `skills.entries.tms.apiKey`, never in prompts.
...
2. **Configure environment**:
   ...
   - `TMS_API_KEY` — **required when TMS has API auth enabled.** Ansible-generated installs set it by default. Configure it in OpenClaw skill env or `skills.entries.tms.apiKey`; the agent should not ask the user to paste it into chat.

   In `~/.openclaw/openclaw.json` you can set per-skill env. Prefer either `skills.entries.tms.env.TMS_API_KEY` or `skills.entries.tms.apiKey` because the skill declares `TMS_API_KEY` as its primary env:
   ```json5
   {
     skills: {
       entries: {
         tms: {
           enabled: true,
           env: { TMS_API_URL: "http://127.0.0.1:8080" },
           apiKey: "..."
           // Or: apiKey: { source: "env", provider: "default", id: "TMS_API_KEY" }
         }
        }
     }
   }
...
- **401 Unauthorized** — `TMS_API_KEY` is missing from OpenClaw skill env or does not match the key in TMS config. Configure the key; do not paste it into chat.
...
3. Publish:
   `clawhub skill publish ./openclaw-skill-tms --slug tms --name "TMS (Telegram Media Server)" --version 1.0.9 --changelog "Require configured TMS API key usage in OpenClaw skill" --tags latest`
