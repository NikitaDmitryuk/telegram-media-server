...
type Database interface {
	...
	Login(ctx context.Context, password string, chatID int64, userName string, config *domain.Config) (bool, error)
...
func InitDatabase(config *domain.Config) error {
	database := NewSQLiteDatabase()
	if err := database.Init(config); err != nil {
		logger.Log.WithError(err).Error("Failed to initialize the database")
		return err
	}
