...
func InitDownloadManager(cfg *domain.Config) {
	GlobalDownloadManager = NewDownloadManager(cfg, database.GlobalDB)
}

func NewDownloadManager(cfg *domain.Config, db database.Database) *DownloadManager {
	dm := &DownloadManager{
...
func (dm *DownloadManager) StartDownload(
	...
	if err != nil {
		logger.Log.WithError(err).Warn("Failed to get file size, using 0")
		fileSize = 0
	}
...
func (dm *DownloadManager) StopDownload(movieID uint) error {
	...
	if exists {
		logger.Log.WithField("movie_id", movieID).Info("Stopping active download")
		if err := job.downloader.StopDownload(); err != nil {
			logger.Log.WithError(err).WithField("movie_id", movieID).Warn("Issue stopping downloader (may have stopped anyway)")
		}
		job.cancel()
		return nil
	}
	...
	if dm.RemoveFromQueue(movieID) {
		logger.Log.WithField("movie_id", movieID).Info("Removed download from queue")
		return nil
	}

	// It's normal for completed downloads to not be found in active downloads or queue
	logger.Log.WithField("movie_id", movieID).Debug("Download not found in active downloads or queue (likely already completed)")
	return nil
}
...
func (dm *DownloadManager) StopAllDownloads() {
	...
	for movieID, job := range jobs {
		logger.Log.WithField("movie_id", movieID).Info("Stopping download")
		if err := job.downloader.StopDownload(); err != nil {
			logger.Log.WithError(err).WithField("movie_id", movieID).Warn("Issue stopping downloader (may have stopped anyway)")
		}
		job.cancel()
...
// GetQueueStatus возвращает статус очереди загрузок
func (dm *DownloadManager) GetQueueStatus() []domain.QueueItem {
	dm.mu.Lock()
	defer dm.mu.Unlock()

	items := make([]domain.QueueItem, 0, len(dm.queue))
	for i, queuedDownload := range dm.queue {
		items = append(items, domain.QueueItem{
			MovieID:   queuedDownload.movieID,
			Title:     queuedDownload.title,
			ChatID:    queuedDownload.chatID,
			Position:  i + 1,
			StartTime: queuedDownload.addedAt, // Используем время добавления в очередь
		})
	}
