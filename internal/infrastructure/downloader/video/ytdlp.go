...
import (
	...
	tmsutils "github.com/NikitaDmitryuk/telegram-media-server/internal/pkg/utils"
)
...
func NewYTDLPDownloader(botInstance domain.BotInterface, videoURL string, config *domain.Config) domain.Downloader {
	videoTitle, err := getVideoTitle(botInstance, videoURL, config)
	if err != nil {
		logger.Log.WithError(err).Error("Failed to retrieve video title, generating fallback title")
...
func (d *YTDLPDownloader) StartDownload(ctx context.Context) (progressChan chan float64, errChan chan error, err error) {
	...
	if useProxy {
		...
		logger.Log.WithField("proxy", proxy).Infof("Using proxy for URL: %s", d.url)
		cmdArgs = append([]string{"--proxy", proxy}, cmdArgs...)
	} else {
		logger.Log.Infof("No proxy used for URL: %s", d.url)
	}
...
func (d *YTDLPDownloader) monitorDownload(
	...
	case <-ctx.Done():
		logger.Log.Info("yt-dlp process canceled due to context cancellation")
		if d.cmd.Process != nil {
			if killErr := d.cmd.Process.Kill(); killErr != nil {
				logger.Log.WithError(killErr).Warn("Failed to kill yt-dlp process")
			}
		}
		processErr = ctx.Err()
	}
	...
	if processErr != nil {
		if d.stoppedManually || errors.Is(processErr, context.Canceled) || errors.Is(processErr, context.DeadlineExceeded) {
			if errors.Is(processErr, context.DeadlineExceeded) {
				logger.Log.Info("yt-dlp process timed out")
			} else {
				logger.Log.Info("yt-dlp process stopped manually")
			}
			errChan <- nil
		} else {
			logger.Log.WithError(processErr).Errorf("yt-dlp exited with error: %s", stderrOutput)
...
func (d *YTDLPDownloader) StopDownload() error {
	...
	if d.cancel != nil {
		logger.Log.Info("Canceling yt-dlp download context")
		d.cancel()
	}

	if d.cmd != nil && d.cmd.Process != nil {
		logger.Log.Info("Stopping yt-dlp process gracefully")

		// First try graceful termination (SIGTERM on Unix systems)
		if err := d.cmd.Process.Signal(os.Interrupt); err != nil {
			logger.Log.WithError(err).Warn("Failed to send interrupt signal, trying kill")
		...
		select {
		case <-done:
			logger.Log.Info("yt-dlp process exited gracefully")
		case <-time.After(gracefulStopTimeout):
			// If graceful termination didn't work, force kill
			logger.Log.Warn("yt-dlp did not exit gracefully, force killing")
			if killErr := d.cmd.Process.Kill(); killErr != nil {
				logger.Log.WithError(killErr).Warn("Failed to force kill yt-dlp process")
			}

			// Wait a bit more for force kill to take effect
			select {
			case <-done:
				logger.Log.Info("yt-dlp process exited after force kill")
			case <-time.After(forceKillTimeout):
				logger.Log.Warn("yt-dlp process did not exit even after force kill, considering it stopped")
			}
		}
	}
	...
	if err := d.cleanupTempFiles(); err != nil {
		logger.Log.WithError(err).Warn("Failed to cleanup temporary files after stop")
	}
		...
		if strings.Contains(tempFile, "*") {
			...
			for _, match := range matches {
				if removeErr := os.Remove(match); removeErr != nil && !os.IsNotExist(removeErr) {
					logger.Log.WithError(removeErr).Debugf("Failed to cleanup temp file: %s", match)
					errorList = append(errorList, removeErr.Error())
				} else if removeErr == nil {
					logger.Log.Infof("Cleaned up temp file: %s", match)
				}
			}
		} else {
			if removeErr := os.Remove(fullPath); removeErr != nil && !os.IsNotExist(removeErr) {
				logger.Log.WithError(removeErr).Debugf("Failed to cleanup temp file: %s", fullPath)
				errorList = append(errorList, removeErr.Error())
			} else if removeErr == nil {
				logger.Log.Infof("Cleaned up temp file: %s", fullPath)
			}
		}
	}
...
func (d *YTDLPDownloader) GetFileSize() (int64, error) {
	useProxy, err := shouldUseProxy(d.url, d.config)
	if err != nil {
		logger.Log.WithError(err).Warn("Failed to determine proxy usage for file size check")
		return 0, nil
	}
	...
	if err != nil {
		logger.Log.WithError(err).Warn("Failed to get video metadata for file size")
		return 0, nil
	}

	var info map[string]any
	if err := json.Unmarshal(output, &info); err != nil {
		logger.Log.WithError(err).Warn("Failed to parse video metadata JSON")
		return 0, nil
	}
...
func shouldUseProxy(rawURL string, cfg *domain.Config) (bool, error) {
	if cfg.Proxy == "" {
		return false, nil
	}
...
func getVideoTitle(_ domain.BotInterface, videoURL string, cfg *domain.Config) (string, error) {
